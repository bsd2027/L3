...
> **注意**
>
> DragonFly BSD 的文档相对陈旧，但这并不能反映其实际开发进度。事实上，DragonFly BSD 的开发仍然比较活跃，不应因官方文档的陈旧而被劝退。
...
U 盘安装应使用 `USB: dfly-x86_64-6.4.0_REL.img as bzip2 file`：先解压出 `dfly-x86_64-6.4.0_REL.img`，然后使用 Rufus 将其刻录到 U 盘。
...
在 `/boot/loader.conf` 中设置开机自动加载 DRM 内核模块，以启用显卡直接渲染
